...
export default function BlogSlugView({ blog, blogsOthers, data, menu, headerMenu }: BlogSlugViewProps) {
  ...
  return (
    ...
    >
      <TitleSection text={blog.attributes.Title as string}> </TitleSection>
      <Line variantColor='grey' />
      <BreadCrumbs links={breadCrumbsItems} />
      <IntroNews blog={blog} />
      {blog.attributes.Text && (
        <div className=' container pt-12 !max-w-screen-lg markDown opacityMarkdown'>
          <ReactMarkdown>{blog.attributes.Text as string}</ReactMarkdown>
        </div>
      )}

      {blog.attributes.Advertise && (
        <div className='container pt-12 !max-w-screen-lg markDown opacityMarkdown'>
          <ReactMarkdown>{blog.attributes.Advertise as string}</ReactMarkdown>
        </div>
      )}
      {blog.attributes.PhotoSlides && <CarouselNews slides={blog.attributes.PhotoSlides as unknown[]} blog={blog} />}
      {blog.attributes.Text2 && (
        <div className=' container pt-12 !max-w-screen-lg markDown opacityMarkdown'>
          <ReactMarkdown>{blog.attributes.Text2 as string}</ReactMarkdown>
        </div>
      )}
      {(blog.attributes.File as { data?: unknown })?.data && (
        <LoadFileBlock file={(blog.attributes.File as { data: unknown }).data} />
      )}
      {blog.attributes.Quote && <QuoteBlock quote={blog.attributes.Quote as Record<string, unknown>} />}

      {(blog.attributes.Video as { data?: unknown })?.data && (
        <div className='pt-[52px]'>
          <Video poster={blog.attributes.Poster} videofile={blog.attributes.Video} />
        </div>
      )}

      {blog.attributes.Text3 && (
        <div className=' container pt-12 !max-w-screen-lg markDown opacityMarkdown'>
          <ReactMarkdown>{blog.attributes.Text3 as string}</ReactMarkdown>
        </div>
      )}
      <BlogsBlockList articleColor='inherit' titleColor='black' buttonColor='black' blogRes={blogsOthers} titleOthers={true} />
      <Line variantColor='grey' />
    </Layout>
  );
}
