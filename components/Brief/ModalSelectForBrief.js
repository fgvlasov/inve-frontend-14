...
export default function ModalSelectForBrief({ options, name, id = "" }) {
  ...
  return (
    <div ref={rootRef} className='relative w-full text-fiord group lg:max-w-[422px]'>
      ...
      >
        ...
        >
          <path fillRule='evenodd' clipRule='evenodd' d='M7 10L12 15L17 10H7Z' fill='#737373' />
        </svg>
      </div>
      <ul className={`select__list ${isOpen ? "block" : "hidden"} absolute top-100 left-0 right-0 bg-white z-50 shadow-2xl`}>
        {options.map((elem) => (
          ...
          >
            <RadioButton text={elem.attributes.Title} checked={isChecked(elem.attributes.Title)} />
          </li>
        ))}
      </ul>
    </div>
