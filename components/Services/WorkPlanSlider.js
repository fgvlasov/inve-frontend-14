...
function SlidePaginationButton({ onClick, direction }) {
  return (
    <div
      className={classNames("relative rounded-full w-10 h-10 md:w-15 md:h-15 lg:w-9 lg:h-9 cursor-pointer block z-10 select-none", {
        "bg-blue": !direction,
        "bg-white": direction,
      })}
      onClick={onClick}
    >
      <img
        src={direction ? "/image/svg/arrow-left.svg" : "/image/svg/arrow-right.svg"}
        ...
        {items?.map((item, key) => (
          ...
          >
            <div className='bg-white rounded-2xl p-10 w-[350px] h-[337px] md:w-[560px] md:h-[301px] duration-500'>
              ...
              <h3 className='text-black font-arial font-normal text-1xl leading-8 mt-5'>{item.title}</h3>
